...
function updateSimReadout() {
  ...
  const tEl = $('#readoutTemp');
  if (tEl) tEl.textContent = `${temp.toFixed(1)}°C`;
  const rEl = $('#readoutRam');
  if (rEl) rEl.textContent = `${ram}%`;
  const bEl = $('#readoutBatt');
  if (bEl) bEl.textContent = `${batt}%`;
...
function syncSimSliders() {
  const simTemp = $('#simTemp');
  if (simTemp) simTemp.value = deviceTelemetry.temperature_c;
  const simTempLabel = $('#simTempLabel');
  if (simTempLabel) simTempLabel.textContent = `${deviceTelemetry.temperature_c.toFixed(1)}°C`;

  const simRam = $('#simRam');
  if (simRam) simRam.value = Math.round(deviceTelemetry.ram_used_percent);
  const simRamLabel = $('#simRamLabel');
  if (simRamLabel) simRamLabel.textContent = `${Math.round(deviceTelemetry.ram_used_percent)}%`;

  const simBatt = $('#simBatt');
  if (simBatt) simBatt.value = Math.round(deviceTelemetry.battery_percent);
  const simBattLabel = $('#simBattLabel');
  if (simBattLabel) simBattLabel.textContent = `${Math.round(deviceTelemetry.battery_percent)}%`;
}
...
// ==========================================
// Native Android Bridge & Mobile Integration
// ==========================================
function initNativeAndroidBridge() {
  if (typeof window.KatsuBridge !== 'undefined' && window.KatsuBridge.isNative && window.KatsuBridge.isNative()) {
    document.body.classList.add('apk-mode');
    console.log('[KATSU APK] Native Android Bridge connected.');

    try {
      // 1. Read real device battery
      const battRaw = window.KatsuBridge.getBatteryInfo();
      if (battRaw) {
        const batt = JSON.parse(battRaw);
        if (typeof batt.level === 'number' && batt.level >= 0) {
          deviceTelemetry.battery_percent = batt.level;
          deviceTelemetry.is_charging = !!batt.isCharging;
        }
      }

      // 2. Read real device memory
      const memRaw = window.KatsuBridge.getMemoryInfo();
      if (memRaw) {
        const mem = JSON.parse(memRaw);
        if (mem.totalRamGb && mem.totalRamGb > 0) {
          deviceTelemetry.ram_gb = Math.round(mem.totalRamGb);
          deviceTelemetry.ram_used_percent = Math.round(mem.usedPercent);
        }
      }

      // 3. Read device model info
      const devRaw = window.KatsuBridge.getDeviceInfo();
      if (devRaw) {
        const dev = JSON.parse(devRaw);
        if (dev.model) {
          const modelBadge = document.querySelector('.hero-compact .eyebrow span');
          if (modelBadge) {
            modelBadge.textContent = `${dev.model} (Calibrated)`;
          }
        }
      }
      ...
      console.warn('[KATSU APK] Telemetry read warning:', err);
    }
  }
}
...
function triggerHaptic(ms = 22) {
  try {
    if (window.KatsuBridge && typeof window.KatsuBridge.vibrate === 'function') {
      window.KatsuBridge.vibrate(ms);
...
document.addEventListener('click', (e) => {
  const target = e.target.closest('button, .dock-item, .game-card, .slide-pill');
  if (target) {
    triggerHaptic(18);
  }
});
...
window.handleAndroidBack = function() {
  // 1. Close open modal
  const modal = document.querySelector('.modal-backdrop.show, #explainModal.show');
  if (modal) {
    modal.classList.remove('show');
    return true;
  }
  // 2. If session is running, switch to idle
  if (isSessionActive) {
    stopLiveSession();
    return true;
  }
  // 3. Navigate back to dashboard if on another tab
